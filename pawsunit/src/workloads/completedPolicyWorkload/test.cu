...
static policyMode parse_policy_mode(const std::string& policy) {
    ...
    fprintf(stderr, "Unknown policy: %s. Expected: none, naive, proper\n",
            policy.c_str());
    std::exit(EXIT_FAILURE);
}
...
static Args parse_args(int argc, char** argv) {
    ...
    // Validate early.
    (void)parse_policy_mode(a.policy);
...
static void run_lc(
    ...
    for (int r = 0; r < args.iters; ++r) {
        pm.phaseBegin("LC_REQUEST", "LC", "LONG");

        pm.phaseBegin("LC_PREFILL_SUBMISSION", "LC", "SHORT");
        compute_heavy_kernel<<<blocks, threads, 0, stream>>>(d, args.n, args.lc_inner * 4);
        CUDA_CHECK(cudaGetLastError());
        pm.phaseEnd();

        pm.phaseBegin("LC_PREFILL_SYNC", "LC", "SHORT");
        CUDA_CHECK(cudaStreamSynchronize(stream));
        pm.phaseEnd();

        pm.phaseBegin("LC_DECODE_STEP", "LC", "SHORT");
...
static void run_be_long(
    ...
    for (int i = 0; i < args.iters; ++i) {
        pm.phaseBegin("BE_LONG_BATCH", "BE", "LONG");
...
static void run_be_chunked(
    ...
    for (int i = 0; i < args.iters; ++i) {
        pm.phaseBegin("BE_CHUNKED_BATCH", "BE", "SHORT");

        for (int c = 0; c < args.chunks; ++c) {
            pm.phaseBegin("BE_CHUNK", "BE", "SHORT");
...
int main(int argc, char** argv) {
    ...
    policyMode selectedPolicy = parse_policy_mode(args.policy);

    fprintf(stderr,
            "class=%s mode=%s policy=%s iters=%d chunks=%d n=%d lc_inner=%d be_inner=%d\n",
            args.klass.c_str(),
            args.mode.c_str(),
            args.policy.c_str(),
            args.iters,
    ...
    pm.setPolicyMode(selectedPolicy);
    ...
    pm.phaseBegin("CLEANUP", "UNK", "SHORT");
    CUDA_CHECK(cudaStreamDestroy(stream));
    CUDA_CHECK(cudaFree(d));
    pm.phaseEnd();
    
    pm.policyManagerHandler->readPolicyData("end_of_workload");
