#!/usr/bin/env python3
"""
Week 2 LC/BE phase-log parser.

What this script produces:
  1. Per-run phase duration tables.
  2. LC_REQUEST comparison table.
  3. BE throughput comparison table with aggregate and per-process throughput.
  4. LC/BE overlap CSVs per colocated run.
  5. A compact overlap summary table.
  6. A compact condition readout table.
  7. CSV summaries under <output_dir>/analysis/.

Expected event line format:
  Event type = BEGIN: PhaseId:[PID, PHASE_ID],Thread Id: TID, parent_id: ..., depth: ..., Timestamp: SEC s NSEC ns, phase type: (...), workload class: LC
