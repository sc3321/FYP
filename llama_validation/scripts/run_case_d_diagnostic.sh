...
start_lc_server() {
  ...
  "$SERVER" \
    --model "$MODEL" --host 127.0.0.1 --port "$LC_PORT" \
    -ngl "$N_GPU_LAYERS" -np 1 \
...
start_be_server() {
  ...
  "$SERVER" \
    --model "$MODEL" --host 127.0.0.1 --port "$BE_PORT" \
    -ngl "$N_GPU_LAYERS" -np 1 \
    > "${log_dir}/server_stdout.log" 2> "${log_dir}/server_stderr.log" &
  BE_PID=$!
...
warmup_client() {
  local port="$1"; local n="$2"; local label="$3"
...
run_client() {
  local port="$1"; local kind="$2"; local n="$3"; local out="$4"
  local out_dir; out_dir="$(dirname "$out")"
  local resp_dir="${out_dir}/responses_${kind}"
  mkdir -p "$out_dir" "$resp_dir"; : > "$out"
  ...
  for i in $(seq 1 "$n"); do
    ...
    request_payload "$kind" | curl -fsS "http://127.0.0.1:${port}/completion" \
      -H "Content-Type: application/json" -d @- > "$response_file" || true
