#!/usr/bin/env python3
"""
Parse phase event logs from policy experiment output directories.

Supported layouts:

  Old layout:
    ROOT/run_name/lc_events/*
    ROOT/run_name/be_events/*
    ROOT/run_name/be1_events/*
    ROOT/run_name/events/*

  New policy layout from the policy runner:
    ROOT/none/lc_events/*
    ROOT/none/be_long_events/*
    ROOT/none/be_chunked_events/*

    ROOT/naive/lc_events/*
    ROOT/naive/be_long_events/*
    ROOT/naive/be_chunked_events/*

The parser adds a `policy` column to summary CSVs.
Policy is inferred in this order:
  1. POLICY=... in config.txt
  2. directory name if it is one of: none, naive, proper, cap
  3. policy_<name> / <name>_policy style names
  4. UNK
"""
...
KNOWN_POLICIES = {"none", "naive", "proper", "cap"}


# ---------------------------------------------------------------------------
# Policy/run discovery
# ---------------------------------------------------------------------------


def infer_policy(run_dir: Path) -> str:
    """
    Infer policy name from config.txt first, then directory name.
    """
    ...
    m = re.search(r"(?:^|[_-])policy[_-]([A-Za-z0-9]+)(?:$|[_-])", name)
    if m:
        return m.group(1)

    m = re.search(r"(?:^|[_-])(none|naive|proper|cap)(?:$|[_-])", name)
    if m:
        return m.group(1)
...
def discover_run_dirs(root: Path) -> List[Path]:
    """
    A run dir is any immediate child of root that contains event directories,
    excluding analysis/. This handles the new layout:

      root/none/lc_events
      root/naive/lc_events

    It also preserves the old behavior where each immediate child is a run.
    """
    return sorted([
...
def is_event_log_path(path: Path) -> bool:
    ...
    if (
        "events" in parts
        or "lc_events" in parts
        or "be_events" in parts
        or "be_long_events" in parts
        or "be_chunked_events" in parts
    ):
        return True
...
def summarise_by_phase(phases: List[Dict]) -> List[Dict]:
    ...
    for p in phases:
        key = (p["class"], p["phase_type"], p.get("granularity", "UNK"))
        by_key.setdefault(key, []).append(p["duration_ms"])

    rows: List[Dict] = []
    for (klass, phase_type, granularity), vals in sorted(by_key.items()):
        rows.append({
            "class": klass,
...
def lc_request_summary(run_name: str, policy: str, phases: List[Dict]) -> Optional[Dict]:
    vals = [
        p["duration_ms"]
        for p in phases
    ...
    return {"run": run_name, "policy": policy, **phase_stats(vals)}


def lc_phase_summary_rows(run_name: str, policy: str, phases: List[Dict]) -> List[Dict]:
    rows: List[Dict] = []
...
def be_work_summary(run_name: str, policy: str, phases: List[Dict]) -> Optional[Dict]:
...
def generate_overlap_csv(run_name: str, policy: str, phases: List[Dict], out_dir: Path) -> None:
    lc_phases = [
        p for p in phases
    ...
    path = out_dir / f"{safe_run}_overlap.csv"
...
def overlap_summary(run_name: str, policy: str, phases: List[Dict]) -> Optional[Dict]:
    lc_requests = [
        p for p in phases
        if p["class"] == "LC" and p["phase_type"] == "LC_REQUEST"
    ]
...
def print_phase_table(rows: List[Dict], title: str) -> None:
    ...
    header = (
        f"{'class':<6} {'phase_type':<28} {'gran':<8} {'count':>6} "
    ...
    for r in rows:
        print(
            f"{r['class']:<6} {r['phase_type']:<28} {r['granularity']:<8} {r['count']:>6} "
...
def print_policy_lc_comparison(rows: List[Dict]) -> None:
    print()
    print("#" * 125)
    print("POLICY LC REQUEST COMPARISON")
    print("#" * 125)
    ...
    baseline_rows = [r for r in rows if r.get("policy") == "none"]
    if not baseline_rows:
        print("No policy=none baseline found; printing raw LC rows only.")
        baseline_p95 = 0.0
        baseline_p99 = 0.0
    else:
        baseline_p95 = median(r["p95_ms"] for r in baseline_rows)
        baseline_p99 = median(r["p99_ms"] for r in baseline_rows)

    header = (
        f"{'policy':<10} {'run':<30} {'count':>6} "
        f"{'mean':>9} {'p50':>9} {'p95':>9} {'p99':>9} {'max':>9} "
        f"{'p95/none':>10} {'p99/none':>10}"
    )
    print(header)
    print("-" * len(header))

    for r in sorted(rows, key=lambda x: (x.get("run", ""), x.get("policy", ""))):
        p95x = r["p95_ms"] / baseline_p95 if baseline_p95 > 0 else 0.0
        p99x = r["p99_ms"] / baseline_p99 if baseline_p99 > 0 else 0.0

        print(
            f"{r.get('policy', 'UNK'):<10} {r['run']:<30} {r['count']:>6} "
            f"{r['mean_ms']:>9.3f} {r['p50_ms']:>9.3f} {r['p95_ms']:>9.3f} "
            f"{r['p99_ms']:>9.3f} {r['max_ms']:>9.3f} "
            f"{p95x:>10.3f} {p99x:>10.3f}"
        )

    if baseline_rows:
        print()
        print(
            f"policy=none median baseline: P95={baseline_p95:.3f} ms, P99={baseline_p99:.3f} ms"
        )


def print_lc_comparison(rows: List[Dict]) -> None:
    """
    Backwards-compatible LC comparison. If policy columns exist, use policy-aware comparison.
    """
    if rows and "policy" in rows[0]:
        print_policy_lc_comparison(rows)
        return
...
def print_be_comparison(rows: List[Dict]) -> None:
    ...
    header = (
        f"{'policy':<10} {'run':<30} {'be_type':<24} {'BE_procs':>8} {'count':>6} "
    ...
    for r in sorted(rows, key=lambda x: (x.get("run", ""), x.get("policy", ""))):
        print(
            f"{r.get('policy', 'UNK'):<10} {r['run']:<30} {r['be_phase_types']:<24} "
...
def print_overlap_comparison(rows: List[Dict]) -> None:
    ...
    header = (
        f"{'policy':<10} {'run':<30} {'LC_p95':>8} {'LC_p99':>8} "
    ...
    for r in sorted(rows, key=lambda x: (x.get("run", ""), x.get("policy", ""))):
        print(
            f"{r.get('policy', 'UNK'):<10} {r['run']:<30} "
...
def print_condition_readout(lc_rows: List[Dict], be_rows: List[Dict]) -> None:
    ...
    baseline_rows = [r for r in lc_rows if r.get("policy") == "none"]
    if not baseline_rows:
        print("No policy=none baseline rows.")
        return

    base_p95_median = median(r["p95_ms"] for r in baseline_rows)
    base_p99_median = median(r["p99_ms"] for r in baseline_rows)
    be_by_key = {(r["run"], r.get("policy", "UNK")): r for r in be_rows}
    ...
    for lc in sorted(lc_rows, key=lambda x: (x.get("run", ""), x.get("policy", ""))):
        policy = lc.get("policy", "UNK")
        be = be_by_key.get((lc["run"], policy))
    ...
    print()
    print(
        f"policy=none median baseline used here: P95={base_p95_median:.3f} ms, P99={base_p99_median:.3f} ms"
    )
...
def main() -> None:
    ...
    ap.add_argument("output_dir", help="Top-level output dir containing policy/run subdirectories")
    ap.add_argument(
        "--quiet-runs",
        action="store_true",
    ...
    for run_dir in run_dirs:
        ...
        if not args.quiet_runs:
            print()
            print("#" * 100)
            print(f"RUN: {run_name}  POLICY: {policy}")
            print("#" * 100)
            print(
            ...
            print_phase_table(summarise_by_phase(phases), f"Phase duration summary: {run_name} [{policy}]")

        lc = lc_request_summary(run_name, policy, phases)
        if lc:
            lc_rows.append(lc)

        lc_phase_rows.extend(lc_phase_summary_rows(run_name, policy, phases))

        be = be_work_summary(run_name, policy, phases)
        if be:
            be_rows.append(be)

        # New policy runner dirs are usually named "none" / "naive", not "lc_vs...".
        # Generate overlap whenever a run has both LC_REQUEST and top-level BE phases.
        if any(p["class"] == "LC" and p["phase_type"] == "LC_REQUEST" for p in phases) and select_top_level_be_phases(phases):
            generate_overlap_csv(run_name, policy, phases, analysis_dir)
            ov = overlap_summary(run_name, policy, phases)
            if ov:
